...
let EmailService = EmailService_1 = class EmailService {
    ...
    async sendStageRequestConfirmation(email, stageRequest) {
        if (!this.transporter) {
    ...
    async sendStageRequestApproval(email, stageRequest) {
        if (!this.transporter) {
            this.logger.error('Service email non initialisé');
            return;
        }
    ...
    async sendStageRequestRejection(email, stageRequest, motif) {
        if (!this.transporter) {
            this.logger.error('Service email non initialisé');
            return;
        }
